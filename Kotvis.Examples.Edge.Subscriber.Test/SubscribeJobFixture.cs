...
{
    [TestClass]
    public class SubscribeJobFixture
    {
        private Module _module;
        private JobDependencyLocator _jobDependencyLocator;
        public SubscribeJobFixture()
        {
        ...
        public SubscribeJob CreateJob(Publisher publisher)
        {
            return new SubscribeJob(_jobDependencyLocator, publisher);
        }
        ...
        public async Task PublisherStateIsSubscribedAfterJobIsRun()
        {
            ...
            await job.Run();
            Assert.AreEqual(ActualPublisherState.Subscribed, publisher.ActualState);
        }

        [TestMethod]
        public async Task PublisherSubscriptionIdIsSetAfterJobIsRun()
        {
            var publisher = CreatePublisher();
            _module.Publishers.Add(publisher);
            var job = CreateJob(publisher);
            await job.Run();
            Assert.AreEqual(publisher.Id, publisher.SubscriptionId);
        }
    }
}
